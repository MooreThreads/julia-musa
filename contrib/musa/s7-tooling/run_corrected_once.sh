...
evidence="$tooling/evidence-s7r2"
sampler="$tooling/telemetry_sample.awk"
collector="$tooling/sample_procfs_once.sh"
expected_hash=900f21e154fe092b572d30236fee343b3b1898d888a13144ab6537a01b645965
timeout_seconds=600

if [[ -e "$evidence" ]]; then
    echo "refusing to replay or overwrite S7R2 evidence: $evidence" >&2
    exit 70
fi
...
sha256sum "$sampler" "$collector" "$tooling/test_telemetry.sh" \
    > "$evidence/frozen-sampler.sha256"
{
    echo 'proc_util_columns=Pid VPid Total 2D TA 3D CMP TRANSFER PidName'
    for proc_util in /proc/driver/musa/gpu*/proc_util; do
        ...
        printf '%s/devname=' "$device_dir"
        sed -n '1p' "$device_dir/devname"
    done
} > "$evidence/frozen-procfs-contract.log"
...
if [[ "$identity_ok" -eq 1 ]]; then
    while kill -0 "$launcher_pid" 2>/dev/null; do
        activity_line=$("$collector" "$launcher_pid")
        sampler_exit=$?
        if [[ "$sampler_exit" -eq 0 ]]; then
            printf '%s\n' "$activity_line" \
                > "$evidence/nonzero-assigned-device-activity.log"
            activity_found=1
            break
        elif [[ "$sampler_exit" -ne 1 ]]; then
            sampler_ok=0
            printf 'sampler_exit=%d\n' "$sampler_exit" \
                > "$evidence/sampler-error.log"
            break
        fi
    done
fi
...
result=INCONCLUSIVE
if [[ "$launcher_exit" -ne 0 || "$sampler_ok" -ne 1 ]]; then
    result=NO_GO
...

{
    ...
    echo "sampler_contract=$sampler_ok"
